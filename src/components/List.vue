    ...
    <div>
        ...
        <md-dialog :md-active.sync="showDialog">
                 ...
                 <div style="width:50%">
                    <p style="padding:10px;font-size:20px" >
                        ...
                        Chiama ora <md-icon>phone</md-icon>
                         ...
                         <md-button style="align-self:center;margin-left:15px" @click="fav(selected)"><md-icon :style="{'color': selected.favorites === true ? 'red' : ''  }">favorite</md-icon></md-button>
                    </p>

                    <div style="display:flex; padding:10px;font-size:18px">
                        Lascia un voto: <br>
                        <li @click="setStar(1)" style="list-style-type:none"><md-icon style="margin-left:10px" :style="{'color': stars >= 1 ? 'yellow' : ''  }">star</md-icon></li>
                        <li @click="setStar(2)" style="list-style-type:none"><md-icon style="margin-left:10px" :style="{'color': stars >= 2 ? 'yellow' : ''  }">star</md-icon></li>
                        <li @click="setStar(3)" style="list-style-type:none"><md-icon style="margin-left:10px" :style="{'color': stars >= 3 ? 'yellow' : ''  }">star</md-icon></li>
                        <li @click="setStar(4)" style="list-style-type:none"><md-icon style="margin-left:10px" :style="{'color': stars >= 4 ? 'yellow' : ''  }">star</md-icon></li>
                        <li @click="setStar(5)" style="list-style-type:none"><md-icon style="margin-left:10px" :style="{'color': stars >= 5 ? 'yellow' : ''  }">star</md-icon></li>   
         ...
         <md-dialog :md-active.sync="showStarConfirm">
             <md-dialog-title style="padding:10px;font-size:25px;text-align:center">Conferma voto</md-dialog-title>
             <div v-if="selected">
            Hai votato {{selected.title.it}} con {{stars}} stelle! <br>
...
data() {
 ...
 computed: {
  ...
  created() {this.load(localStorage.getItem('toSearch'))},
  methods: {
      load(val) {
      ...
      logId(val) {  
        ...
        //get voti
        db.collection('votes').where('restid', '==', this.selected.id).get().then(function(querySnapshot) {
            var s = 0
            if (!querySnapshot.empty) {
                querySnapshot.forEach((d) => {s += d.data().stars})
                ...
                if (vueInstance.user.loggedIn == true) {
                    db.collection('votes').where("usermail", '==', vueInstance.user.data.email).where("restid", '==', vueInstance.selected.id).get().then(function(querySnapshot) {
                    if (!querySnapshot.empty) {
                        querySnapshot.forEach((d) => {vueInstance.selected.docid = d.id})
                    }
                })
                }
        ...
        if (this.user.loggedIn == true) {
          db.collection('favorites').where('restid', '==', this.selected.id).where("usermail", '==', this.user.data.email).get().then(function(querySnapshot) {
            if (!querySnapshot.empty) {
      ...
      sendVote() {
            ...
            if (this.user.loggedIn == false) {
                ...
                if (this.selected.docid) {
                    db.collection('votes').doc(this.selected.docid).update({stars: this.stars}).then(() => {this.showStarConfirm = false; this.showDialog = false})
                } else {
          ...
          const db = firebase.firestore()
            if (this.user.loggedIn == false) {
                ...
                if (!v.favorites) {
                    ...
                    db.collection('favorites').doc(v.docfav).delete().then(() => {
                        this.showDialog = false
                    }).catch(err => {
                        console.log(err)
                    })
